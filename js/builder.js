...
for (var i=0; i < items_arr.length; i++) {
	...
	for (var j=0; j < levels_per_item; j++) {
		html += "<td><input type=\"button\" title='increase production rate by " + numberFormat(prestigeMultiplier() * calc(j)) + '/s for ' + items_arr[i] + "' onclick=\"rateInc('" + items_arr[i] + "', "+ j +");\" value='" + numberFormat(prestigeMultiplier() * calc(j)) +"/s'></td>";
	}

    html += "<td><input type=\"button\" title='increase production rate for " + items_arr[i] + " by max available " + next_map[items_arr[i]] + "' onclick=\"buildAllRateInc('" + items_arr[i] + "');\" value=\"ALL\">";
